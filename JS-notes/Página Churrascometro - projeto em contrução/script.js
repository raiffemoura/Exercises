...
function calc() {
    ...
    if (adu.length == 0 || cri.length == 0 || horas.length == 0) {
        ...
        var carne = adu * 0.400 + cri * 0.200
        var cerveja = adu * 1.2 + cri * 0.6
        var refri = adu * 1 + cri * 0.5
        if (horas <= 6) {

            res.innerHTML = `Você vai precisar de ${carne.toFixed(3)}Kg de carne e ${cerveja.toFixed(1)}L de cerveja e 
                        ${refri.toFixed(1)}L de refrigerente e água`

        } else {
        
        var carne = adu * 0.650 + cri * 0.325
        var cerveja = adu * 2 + cri * 1
